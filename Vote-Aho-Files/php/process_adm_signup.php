...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (pg_num_rows($check_tribID) > 0) {
        ...
        echo "<script>alert('Account registered already, like me for frybread tasting at the powwow!'); window.location.href = 'admhome.php';</script>";
        exit();
    } else {
	...
	// verifies that both tables were updated successfully
        if ($result and $result2) {
            ...
            echo "<script>alert('Account Created! Aho!'); window.location.href = 'admhome.php';</script>";
            exit();
        } else {
            ...
            echo "<script>alert('ERROR'); window.location.href = 'add_vot.php';</script>";
            exit();
        }
    }
